...
- A 180×42 tmux TUI proof showed both `/m ` and `/pr ` dropdowns listing staged current, sibling-worktree, and nested-repository targets. A second normal global-extension session ran `/reload` and showed the live `/m ` dropdown from the canonical symlinked source.
